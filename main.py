...
from api_client import generate_response, CodeSnippet, StringResponse, USE_LOCAL_MODEL
...
def process_task(task: str, builder: SnippetBuilder, context: dict, system_prompt: str, local_tracker: TokenTracker, logger: logging.Logger) -> str:
    ...
    for i, step in enumerate(steps, 1):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.error(f"Error generating code for step {i}: {e}")
                full_response += f"## Step {i}: {step}\n\nError generating code: {e}\n"
                break
            if not code_snippet:
                logger.warning(f"No code generated for step {i}")
                full_response += f"## Step {i}: {step}\n\nNo code generated\n"
                break
            ...
            if valid:
                ...
                logger.warning(f"Failed after {max_attempts} attempts for step {i}: {result}")
...
def main():
    ...
    while True:
        try:
            ...
            logger.info(f"Loaded context with {len(context.get('completed_tasks', []))} completed tasks")
            if task_queue.empty():
                ...
                logger.info("Generating next task via Mega prompt")
                ...
                logger.info(f"Received reply from {model_used}: {reply}")
                if reply.startswith("UPDATE PROMPT:"):
                    ...
                    logger.info(f"Updated system prompt with: {instruction}")
                else:
                    task_queue.put(reply)
                    logger.info(f"Added task to queue: {reply}")
            while not task_queue.empty():
                task = task_queue.get()
                logger.info(f"Processing task: {task}")
                full_response = process_task(task, builder, context, system_prompt, local_tracker, logger)
                # Generate markdown file for the task
                task_seq = len(os.listdir("tasks"))
                filename = f"tasks/task_{task_seq:03d}.md"
                with open(filename, "w") as f:
                    f.write(full_response)
                logger.info(f"Saved task output to {filename}")
